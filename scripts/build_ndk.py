...
TARGET_DIR: str = os.path.join(
    os.path.dirname(CWD), "libwoff2dec", "src", "main", "cpp", "thirdparty"
)
...
def fetch_woff2_sources() -> None:
    ...
    run(f"git clone --single-branch --branch {WOFF2_VERSION} --recursive {WOFF2_REPO}")
...
def build_woff2() -> None:
    ...
    for ABI in ABI_LIST:
        ...
        run(
            f"cmake -S {BROTLI_SOURCE_DIR} -B {BROTLI_BUILD_PATH}"
        ...
        run(
            f"cmake --build {BROTLI_BUILD_PATH} --config Release --target install -j {CPU_COUNT}"
        )
        ...
        run(
            f"cmake --build {WOFF2_BUILD_PATH} --config Release --target install -j {CPU_COUNT}"
        )
